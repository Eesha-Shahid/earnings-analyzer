...
def finbert_score(text: str) -> dict:
    ...
    try:
        # FinBERT max 512 tokens — truncate to ~350 words to be safe
        words = text.split()[:350]
        text = " ".join(words)
        results = pipe(text, truncation=True, max_length=512)[0]  # ← add truncation=True
...
# Pre-load FinBERT at module import so it's ready before parallel execution
get_finbert()
