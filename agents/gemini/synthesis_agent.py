...
from agents.gemini.query_agent import QueryPlan
from agents.gemini.retrieval_agent import RetrievalResult, RetrievedChunk
from agents.gemini.parallel_runner import SpecialistResults
...
class SynthesisAgent:
    ...
    def run(
        ...
        warnings = []
        try:
            answer = self._call_gemini(system, user_prompt)
            print(f"  ✅ Synthesis complete (Gemini)")
        except Exception as e:
            answer = "Unable to generate answer. Please try again."
            warnings.append(f"Gemini failed: {e}")
